 ...
 * Version 1.0.7
 * 
 * @author duiba fxt 提供券码复制功能接口。 js触发，券码传值 提供监听返回首页时，积分刷新动作。 删除掉不使用的工具方法
 */
...
//        }
    ...
    /**
     * 拦截url请求，根据url结尾执行相应的动作。 （重要） 用途：模仿原生应用体验，管理页面历史栈。
     * 
     * @param view
     * @param url
     * @return
    ...
    private void setListener() {
        if (type == 0)
            return;
        Button button = mTitleBar.getRightBtn();
        button.setText("最新兑换");
        // button.setBackgroundColor(getResources().getColor(R.color.title_red));
        button.setOnClickListener(new OnClickListener() {

            @Override
            public void onClick(View arg0) {
              //  startActivity(ShowNewDuiHuanActivity.class);
            }
        });
    }
